"""`peek` CLI: `doctor`, `list`, `read`, `install-shim` subcommands.

Used for manual development against the live AX API and to set up the
stable shim path that AX trust is granted to.
"""
...
def cmd_doctor(_args: argparse.Namespace) -> int:
    ...
    print(f"current interpreter: {drift['current_path']}")
    ...
    if drift["prior_path"]:
        ...
        if drift["drifted"]:
            ...
            print("         Re-grant Accessibility access to the shim path:")
            print(f"             {SHIM_PATH}")
        else:
            print("trust state:        unchanged since last run")

    shim_exists = SHIM_PATH.exists()
    print(f"shim ({SHIM_PATH}): {'present' if shim_exists else 'MISSING — run `peek install-shim`'}")
    ...
    if not trusted:
        ...
        print(f"Add the shim path to the list: {SHIM_PATH}")
...
SHIM_SCRIPT = """\
#!/usr/bin/env bash
# Stable shim for macos-peek-mcp.
#
# Generated by `peek install-shim`. AX trust is granted to THIS path; the
# real Python interpreter under uv may be re-signed on upgrade and lose
# trust silently, so MCP clients should always point at this shim and
# never directly at the venv interpreter.
#
# Edit the PYTHON line if you move your venv. `peek doctor` checks for
# drift after upgrades.

PYTHON={python}
exec "$PYTHON" -m peek.server "$@"
"""


def cmd_install_shim(args: argparse.Namespace) -> int:
    target = Path(args.path).expanduser() if args.path else SHIM_PATH
    ...
    print("and select the shim. (Spotlight won't find it — you'll need to use")
    print("Cmd+Shift+G in the file picker and paste the path.)")
    return 0
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_doctor = sub.add_parser("doctor", help="Check AX trust state and shim hash")
    ...
    p_install = sub.add_parser(
        "install-shim",
        help=f"Write {SHIM_PATH} so AX trust survives uv upgrades",
    )
    p_install.add_argument("--path", help=f"Override shim location (default {SHIM_PATH})")
    p_install.set_defaults(func=cmd_install_shim)
